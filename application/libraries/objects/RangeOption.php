...
class RangeOption extends DhcpOption {
	...
	public function set_range($new) {
		$this->CI->api->dhcp->modify->rangeoption($this->range, $this->option, $this->value, 'name', $new);
		$this->range = $new;
	}
	
	public function set_option($new) {
		$this->CI->api->dhcp->modify->rangeoption($this->range, $this->option, $this->value, 'option', $new);
		$this->option = $new;
	}
	
	public function set_value($new) {
		$this->CI->api->dhcp->modify->rangeoption($this->range, $this->option, $this->value, 'value', $new);
		$this->value = $new;
	}
