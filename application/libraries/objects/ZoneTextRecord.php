...
class ZoneTextRecord extends DnsRecord {
	...
	public function set_hostname($new) {
		$this->CI->api->dns->modify->zone_text($this->hostname, $this->zone, 'hostname', $new);
		$this->hostname = $new;
	}

	public function set_zone($new) {
		$this->CI->api->dns->modify->zone_text($this->hostname, $this->zone, 'zone', $new);
		$this->zone = $new;
	}

	public function set_ttl($new) {
		$this->CI->api->dns->modify->zone_text($this->hostname, $this->zone, 'ttl', $new);
		$this->ttl = $new;
	}

	public function set_owner($new) {
		$this->CI->api->dns->modify->zone_text($this->hostname, $this->zone, 'owner', $new);
		$this->owner = $new;
	}

	public function set_text($new) {
		$this->CI->api->dns->modify->zone_text($this->hostname, $this->zone, $this->type, 'text', $new);
		$this->owner = $new;
	}
