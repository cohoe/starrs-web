...
class TextRecord extends DnsRecord {
	...
	public function set_hostname($new) {
		$this->CI->api->dns->modify->txt($this->hostname, $this->zone, $this->text, 'hostname', $new);
		$this->hostname = $new;
	}

	public function set_zone($new) {
		$this->CI->api->dns->modify->txt($this->hostname, $this->zone, $this->text, 'zone', $new);
		$this->zone = $new;
	}

	public function set_ttl($new) {
		$this->CI->api->dns->modify->txt($this->hostname, $this->zone, $this->text, 'ttl', $new);
		$this->ttl = $new;
	}

	public function set_owner($new) {
		$this->CI->api->dns->modify->txt($this->hostname, $this->zone, $this->text, 'owner', $new);
		$this->owner = $new;
	}

	public function set_text($new) {
		$this->CI->api->dns->modify->txt($this->hostname, $this->zone, $this->text, 'text', $new);
		$this->owner = $new;
	}
