...
class ClassOption extends DhcpOption {
	...
	public function set_class($new) {
		$this->CI->api->dhcp->modify->classoption($this->class, $this->option, $this->value, 'class', $new);
		$this->class = $new;
	}
	
	public function set_option($new) {
		$this->CI->api->dhcp->modify->classoption($this->class, $this->option, $this->value, 'option', $new);
		$this->option = $new;
	}
	
	public function set_value($new) {
		$this->CI->api->dhcp->modify->classoption($this->class, $this->option, $this->value, 'value', $new);
		$this->value = $new;
	}
